...
const nelMessaggio = estraiSchede(espandiCompatte(om));
const nelTurno = estraiSchede(espandiCompatte(daModello));
...
if (!scelte.length && intent === 'conferma') {
  try {
    ...
    for (const id of Object.keys(mappa)) {
      ...
      if (v.tipo === 'richiesta') continue;   // descritta ma senza scheda: non si salva da sola
