...
const dati = rows
  ...
  .filter(s => s.length > 0)
  .join('\n---\n');
// BOZZE LEDGER v2 — 22/08/2026: all'agente vanno SOLO le bozze aperte.
// Dalla v7 di «Aggiorna Bozze» (22/08 mattina) una scheda confermata dalla ricevuta non
// viene piu cancellata dal registro: resta come lapide con `st:'salvata'`, cosi una
// ristampa del recap non la riapre. Ma qui si iniettava `Object.values(map)` per intero,
// quindi da quel momento l'agente ha ricevuto anche le schede GIA SUL GESTIONALE come se
// fossero bozze da confermare — e non capiva piu quale fosse la bozza vera.
// Prova: 22/08 ore 17:22, l'agente propone di cancellare TR/22082026/P4RII6CXR7K72UVO,
// che era stato scritto sul gestionale alle 17:15 (riga 1360, esecuzione 788297).
// Regola: si inietta solo cio che e ancora aperto. Le lapidi servono al registro, non al
// modello.
let bozze = '';
try {
  for (const r of $('Leggi Bozze').all()) {
    ...
    bozze = Object.values(map)
      .filter(v => v && v.st !== 'salvata')
      .map(v => String((v && v.b) || '')).filter(Boolean).join('\n\n');
  }
} catch (e) {}
